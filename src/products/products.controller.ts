  ...
  return {
    ...
    offers: {
      ...
      priceCurrency:
        ...
        product.variants[0]?.price_set?.shop_money?.currency_code ||
        'USD',
      ...
      offers: product.variants.map((variant) => ({
        ...
        priceCurrency:
          ...
          variant.price_set?.shop_money?.currency_code ||
          'USD',
        availability:
...
export class ProductsController {
  ...
  async findAll(
    ...
    const filters = {
      ...
      tagsAny: tagsIncludeAny
        ? tagsIncludeAny
            .toLowerCase()
            .split(',')
            .map((t) => t.trim())
        : [],
      tagsAll: tagsIncludeAll
        ? tagsIncludeAll
            .toLowerCase()
            .split(',')
            .map((t) => t.trim())
    ...
    const filteredProducts = allProducts.filter((product: any) => {
      // Specify Product type if you have an interface
      ...
      const matchesQ =
        !filters.q ||
        title.includes(filters.q) ||
        description.includes(filters.q);
      const matchesProductType =
        !filters.productType || currentProductType === filters.productType;
      const matchesVendor = !filters.vendor || currentVendor === filters.vendor;
      const matchesTagsAny =
        !filters.tagsAny.length ||
        filters.tagsAny.some((tag) => currentTags.includes(tag));
      const matchesTagsAll =
        !filters.tagsAll.length ||
        filters.tagsAll.every((tag) => currentTags.includes(tag));
      const matchesPriceMin =
        filters.priceMin === null || prices.some((p) => p >= filters.priceMin);
      const matchesPriceMax =
        filters.priceMax === null || prices.some((p) => p <= filters.priceMax);
      const matchesStock =
        filters.inStock === undefined || filters.inStock === hasStock;
      ...
      if (filters.color || filters.size) {
        for (const option of product.options || []) {
          ...
          if (filters.color && optionName.includes('color')) {
            matchesColor = product.variants.some(
              (v) =>
                v[`option${option.position}`]?.toLowerCase() === filters.color,
            );
          }
          if (filters.size && optionName.includes('size')) {
            // Simple check, might need refinement
            matchesSize = product.variants.some(
              (v) =>
                v[`option${option.position}`]?.toLowerCase() === filters.size,
            );
          }
        }
      }
  ...
  async findOne(@Param('id') id: string, @Query('shop') shop: string) {
    ...
    const productShopDomain = shop.split('/')[0];
